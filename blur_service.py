...
def api_config_get():
    cfg = _load_model_config()
    return {
        "detection_interval": cfg.get("detection_interval", _DETECTION_INTERVAL),
        "plate_conf_thresh": cfg.get("plate_conf_thresh", _PLATE_CONF_THRESH),
    }
...
def run_deface(input_path: str, output_path: str, mode: str = "faces",
    ...
    cfg = _load_model_config()
    _det_interval = int(cfg.get("detection_interval", _DETECTION_INTERVAL))
    _conf_thresh  = float(cfg.get("plate_conf_thresh", _PLATE_CONF_THRESH))
    ...
    _set(hw_nvdec=use_nvdec, hw_nvenc=use_nvenc)
